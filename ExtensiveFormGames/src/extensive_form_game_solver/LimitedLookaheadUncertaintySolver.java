...
/**
   This class assumes two types of uncertainty on the node
   evaluation function applied by the limited-lookahead adversary.
   First, an overall node evaluation function is sampled.
   This function gives a mean score for each individual node in
   the game tree.
   Second, at each node, we assume that the exact score of the
   evaluation function is given by a Normal distribution, all with the
   same standard deviation and mean given by the overall node evaluation
   function.
*/
public class LimitedLookaheadUncertaintySolver extends SequenceFormLPSolver {

    // first index denotes the pre-game evaluation function sample.
    // second index denotes the mean evaluation on a particular node.
    double[][] nodeEvaluationMean;
    // the distribution over pre-game sampled evaluation functions
    double[] nodeEvaluationDistribution;
    int lookAhead;

    // standard deviation on each individual node eval
    double stdDev;
    ...
    public LimitedLookaheadUncertaintySolver(Game game, int playerToSolveFor, double[][] nodeEvaluationTable, int lookAhead, double epsilon) {
        super(game, playerToSolveFor);
        this.nodeEvaluationMean = nodeEvaluationTable;
    ...
    public LimitedLookaheadUncertaintySolver(Game game, int playerToSolveFor, double[][] nodeEvaluationTable, int lookAhead) {
        this(game, playerToSolveFor, nodeEvaluationTable, lookAhead, 0.001);
    }

    private void makeNodeEvaluationTableNonNegative() {
        double minValue = 0;
        for (int i = 0; i < nodeEvaluationMean.length; i++) {
            for (int j = 0; j < nodeEvaluationMean[i].length; j++) {
                if (nodeEvaluationMean[i][j] < minValue)
                    minValue = nodeEvaluationMean[i][j];
            }
        }
        for (int i = 0; i < nodeEvaluationMean.length; i++) {
            for (int j = 0; j < nodeEvaluationMean[i].length; j++) {
                nodeEvaluationMean[i][j] += Math.abs(minValue);
            }
        }
    }
